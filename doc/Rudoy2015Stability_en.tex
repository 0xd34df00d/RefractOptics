...
\begin{equation}
  ...
  \label{eq:model_0}
\end{equation}
is suggested by the experts, as physical considerations show that refraction index
$n$ depends on even powers of the wavelength $\lambda$.

The optimal values of parameters vector $\boldsymbol{\omega}$ for the first and
the second polymer are shown in table \ref{tabl:res_even_coeffs}.

\begin{table}[h]
  \centering
  \footnotesize
  \caption{Coefficients of model \eqref{eq:model_0} and their relative residual.}
...
\begin{table}[h]
  \centering
  \footnotesize
  \caption{Standard deviation of \eqref{eq:model_0} parameters for the first polymer for selected noise parameters.}
  \begin{tabular}{l | c | c | c}
...
These results may be a sign of overfitting, and that the resulting model can't
be used to describe the physical process, and can not be used to separate two
