...
We then consider the modified sample set $\acute{D}$ derived from the
...
For this new sample set $\acute{D}$ we find the new parameter vector $\hat{\boldsymbol{\omega}}_f (\acute{D} (\Sigma^{\mathbf{x}}, \boldsymbol{\sigma}_y))$
...
vectors from the sample set, as well as constants which are subject to
...
with slightly bigger sample set errors generalize better.
