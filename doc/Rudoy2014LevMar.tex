...
\begin{center}
  МОДИФИКАЦИЯ ФУНКЦИОНАЛА КАЧЕСТВА В ЗАДАЧАХ НЕЛИНЕЙНОЙ РЕГРЕССИИ ДЛЯ УЧЕТА ПОГРЕШНОСТИ НЕЗАВИСИМЫХ ПЕРЕМЕННЫХ
...
\begin{abstract}
  ...
  с гетероскедастичными погрешностями измерения
...
\begin{itemize}
  \item Критерием останова могут служить обычные критерии, такие как достижение некоторого
    числа итераций, порог порогг нормы изменения вектора $\bomega$, и т.~п.
...
и $\bomega_j^0 = \arg \min S$, что повторяется $N$ раз, и для $\forall i$ рассматриваются значения
\[
...
Таким образом, варьируя $k$ и изучая поведение $\overline{\delta \bomega}$ и
$\overline{\delta \bomega^0}$, можно исследовать влияние погрешности определения
...
\ref{fig:comparison_0.65_alpha0}), посколько в выбранном масштабе они
...
устойчивости к погрешностям как в зависимых, так и независимых
